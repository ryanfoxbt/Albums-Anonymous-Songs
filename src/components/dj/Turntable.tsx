...
export function Turntable({
  ...
  return (
    <div className="relative flex h-40 w-40 shrink-0 items-center justify-center self-center">
      <div
        className="relative h-36 w-36 rounded-full border-4 border-black/20 dark:border-white/10"
        style={{
          background:
      ...
      >
        <div className="absolute left-1/2 top-1/2 flex h-14 w-14 -translate-x-1/2 -translate-y-1/2 items-center justify-center overflow-hidden rounded-full border border-black/40 bg-neutral-700">
          {song?.coverImageUrl ? (
      ...
      <div
        className="absolute right-3 top-1 h-20 w-1.5 origin-top-right rounded-full bg-black/70 transition-transform duration-500 ease-out dark:bg-white/60"
        style={{ transform: isPlaying ? "rotate(25deg)" : "rotate(-8deg)" }}
      >
        <div className="absolute -bottom-1 -left-1.5 h-3 w-4 rounded-sm bg-black/80 dark:bg-white/70" />
      </div>
    </div>
  );
}
